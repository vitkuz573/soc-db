...
from soc_db.common import fetch, slug, write_vendor_file
...
def scrape():
    logger.info("Fetching Apple A-series...")
    a_chips = parse_tables(fetch(WIKI_A), "A")
    logger.info("Fetching Apple M-series...")
...
def main():
    logger.info("Apple Silicon scraper")
    chips = scrape()
    if chips:
